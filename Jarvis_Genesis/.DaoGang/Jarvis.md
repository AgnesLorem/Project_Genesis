...
- **Mandatory Roblox Studio Runtime Verification**: For any changes affecting combat, progression, currency, save/load persistence, UI panels, or network replication, the agent **MUST** run Play Solo mode in Roblox Studio (using the `start_stop_play` tool) and execute a verification/smoke check (e.g. using `GameplaySimulator` or console Luau runs). Offline/RAM-only syntax or schema checks are NOT sufficient. The agent must verify that the code executes correctly without errors or warning leaks in the actual Roblox engine environment, and include the verification logs in their final report.
